...
// [[Rcpp::export]]
double dsgomp(double x, double b, double eta) {
...
// [[Rcpp::export]]
double psgomp(double x, double b, double eta) {
...
// [[Rcpp::export]]
NumericVector rsgomp(int n, double b, double eta) {
  ...
  std::cout << "Mode detected at: " << round(t0 * 1e3) / 1e3 << "\n";
  
  // SG max;
  double m = dsgomp(t0, b, eta);
  std::cout << "Density maximum: " << round(m * 1e3) / 1e3 << "\n";
  
  // exponential denisty cut-off point
  double t1 = - 1 / b * (log(m) - log(1 + eta) - log(b));
  std::cout << "Exponential density cut-off point: " << round(t1 * 1e3) / 1e3 << "\n";
  
  // calculate integral of the density
  double auc = m * t1 + (exp(- t1 * b)) * (1 + eta);
  std::cout << "Integral of un-normalised density: " << round(auc * 1e3) / 1e3 << "\n";
  
  // calculate probability of uniform part of the density
  double p1 = m * t1 / auc;
  std::cout << "Probability of uniform component: " << round(p1 * 1e3) / 1e3 << "\n";
...
system.time(s <- rsgomp(n = n, b = b, eta = eta))
